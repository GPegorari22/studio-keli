-- O painel do aluno lê somente o próprio perfil e seus dados acadêmicos.
-- A função usa o e-mail presente no JWT para funcionar também no primeiro acesso,
-- antes de a administração preencher aluno.id_usuario.
...
begin
  ...
  if id_do_aluno is null then
    raise exception 'Perfil de aluno não encontrado.' using errcode = '42501';
  end if;
  ...
  if not exists (
    ...
    raise exception 'A presença só pode ser confirmada na aula de hoje.' using errcode = '22023';
  end if;
...
revoke all on function public.meu_painel_aluno() from public;
revoke all on function public.confirmar_presenca_da_proxima_aula(integer) from public;
...
comment on function public.meu_painel_aluno() is
  'Retorna somente os dados necessários para o painel do aluno autenticado.';

comment on function public.confirmar_presenca_da_proxima_aula(integer) is
  'Permite ao aluno autenticado confirmar presença apenas na própria aula do dia.';
